...
exports.MotorcoScraper = void 0;
const axios_1 = __importDefault(require("axios"));
const cheerio = __importStar(require("cheerio"));
const BaseScraper_1 = require("./BaseScraper");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class MotorcoScraper extends BaseScraper_1.BaseScraper {
    constructor() {
        const config = {
    ...
    async scrape() {
        try {
            ...
            const ajaxResponse = await axios_1.default.post(ajaxUrl, formData, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
            ...
            if (eventContainers.length > 0) {
                eventContainers.each((i, el) => {
                    ...
                    const performers = [];
                    if (mainTitle && mainTitle !== "Event at Motorco")
                        performers.push(mainTitle);
                    if (supportingActs) {
            ...
            if (scrapedEvents.length === 0) {
                ...
                if (calendarLink) {
                    ...
                    const calendarResponse = await axios_1.default.get(calendarLink, {
                        headers: {
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
                        }
                    });
...
exports.MotorcoScraper = MotorcoScraper;
