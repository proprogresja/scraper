import { RitzRaleighScraper } from '../scrapers/RitzRaleighScraper'
...
async function runRitzRaleighScraper() {
  ...
  const scraper = new RitzRaleighScraper()
  ...
  try {
    ...
    const outputPath = join(__dirname, '..', 'data', 'scrapes', `ritz-raleigh-scrape-${timestamp}.json`)
